...
from backend.src.api.models.schemas.references import Code, Reference, ReferenceRangeItem, TempEnum
from backend.src.api.utils import get_api_key, __check_semantic_interoperable, __check_unit
...
@router.get("/", summary="Returns all observation references", status_code=status.HTTP_200_OK)
def get_references(api_key: APIKey = Depends(get_api_key)) -> dict:
    return resource.references

@router.get("/_keys", summary="Returns all observation reference keys", status_code=status.HTTP_200_OK)
def get_reference_keys(api_key: APIKey = Depends(get_api_key)) -> list:
    return resource.reference_keys

@router.get("/_acronyms", summary="Returns all observation reference acronyms", status_code=status.HTTP_200_OK)
def get_acronyms(api_key: APIKey = Depends(get_api_key)) -> dict:
    return resource.acronyms

@router.get("/_acronyms/{key}", summary="Returns an observation reference by its acronmy", status_code=status.HTTP_200_OK)
...
@router.get("/{key}/_referenceRange", summary="Returns observation reference range by its reference key", status_code=status.HTTP_200_OK, response_model=list[ReferenceRangeItem], response_model_exclude_unset=True)
...
@router.get("/{key}/_code", summary="Returns observation reference code by its reference key", status_code=status.HTTP_200_OK, tags=["References"])
...
@router.post("/{key}", summary="Generate observation result by its reference key", status_code=status.HTTP_201_CREATED)
